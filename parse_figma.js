...
async function fetchFigma() {
  ...
  const url = "https://api.figma.com/v1/files/d6cS6vlKHQNKT6dvMfOOEG/nodes?ids=48:91";
  
  const response = await fetch(url, { headers: { 'X-Figma-Token': token } });
  const data = await response.json();
  
  const root = data.nodes['48:91'].document;
  ...
  fs.writeFileSync('figma_structure.json', JSON.stringify(clean(root), null, 2));
  console.log("Written to figma_structure.json");
}
